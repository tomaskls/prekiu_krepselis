export function shoppingList(list) {
  ...
  const ilgisP = list.reduce((maxLength, item) => 
    Math.max(maxLength, item.name.length), 9) ;
  const ilgisK = list.reduce((maxLength, item) => 
    Math.max(maxLength, item.amount.toString().length), 6) + 4 ;
  ...
  const ilgisV = list.reduce((maxLength, item) => 
    Math.max(maxLength, (item.unitPrice * item.amount).toString().length), 11) + 4;
